#!/usr/bin/env python
"""
09 — Dense Classification Experiment (Chebyshev & Legendre, n=1..25)

Evaluates Chebyshev and Legendre L2-normalised polynomial representations
at n_coeffs = 1, 2, ..., 25 plus the og_xp_110 baseline, using 4 classifiers
and 10x5 Repeated Stratified K-Fold.
...
Parallelism strategy (12-core machine):
  - SVM_RBF:           4 splits in parallel x 3 CV folds = 12 threads
...
Outputs:
  - results/dense_experiment_results.csv
  - results/dense_summary.csv
  - models_dense/*.joblib + model_manifest.json
"""
...
parser = argparse.ArgumentParser(description="Dense classification experiment")
parser.add_argument("--smoke", action="store_true",
                    help="Quick smoke test: 2 reprs, 5 splits, 5 HPO trials")
parser.add_argument("--only", nargs="+", default=None,
                    help="Run only these representations, e.g. --only legendre_5_L2 legendre_10_L2")
...
if SMOKE:
    ...
    print(f"Loaded {len(splits)} splits (10 repeats x 5 folds)")
...
CLASSIFIERS = [
    ...
    },
    {
        ...
        "outer_n_jobs": min(2, max(1, N_CPU // 4)),
        "cv_n_jobs": 3,
    },
    {
        "name": "RandomForest",
...
# ═══════════════════════════════════════════════════════════════════════
# 4. Single-cell worker (one repr x clf x split)
# ═══════════════════════════════════════════════════════════════════════
...
results_path = RESULTS_DIR / ("_smoke_dense_results.csv" if SMOKE else "dense_experiment_results.csv")
completed = set()
...
for repr_i, repr_cfg in enumerate(REPRESENTATIONS, 1):
    ...
    for clf_cfg in CLASSIFIERS:
        ...
        print(
            f"  >> {repr_cfg['name']} x {clf_cfg['name']}: "
...
final_csv = RESULTS_DIR / ("_smoke_dense_results.csv" if SMOKE else "dense_experiment_results.csv")
...
summary_csv = RESULTS_DIR / ("_smoke_dense_summary.csv" if SMOKE else "dense_summary.csv")
...
print(f"Saved {len(model_manifest)} models -> models_dense/")
print(f"Total size: {total_size_mb:.1f} MB")
print(f"\nRun 10_analyze_dense.ipynb for analysis.")
